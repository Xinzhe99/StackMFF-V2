...
Multi-focus image fusion (MFF) is a vital computational technique for applications requiring an extended depth of field, such as professional photography, microscopy, and autonomous driving. While existing methods excel at fusing image pairs, they often suffer from quality degradation and computational inefficiency when applied to large image stacks. To address this challenge, we introduce a novel, ultra-lightweight learning-based framework that reframes the stack fusion task as a focal depth regression problem. This framework comprises three key stages: intra-layer focus estimation, inter-layer focus estimation, and focus map regression. By employing a differentiable soft-regression strategy and utilizing depth maps as proxy supervision signals, our method facilitates end-to-end training while eliminating the requirement for manual focus map annotations. Comprehensive experiments on five public datasets demonstrate that our approach achieves state-of-the-art performance with minimal computational overhead. This significant efficiency and scalability make the proposed framework a compelling solution for real-time deployment in resource-constrained environments, laying the foundation for the broader adoption of multi-focus image fusion in practical applications.
